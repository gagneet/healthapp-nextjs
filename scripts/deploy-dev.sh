...
# HealthApp Development Deployment Script (Legacy)
...
echo ""
echo "🚀 Starting HealthApp Development Deployment (Legacy Mode)..."
...
if [ ! -f env_files/.env.docker.development ]; then
    print_warning "env_files/.env.docker.development not found. Creating from template..."
    if [ -f env_files/.env.development.example ]; then
        cp env_files/.env.development.example env_files/.env.docker.development
        print_warning "⚠️  IMPORTANT: Edit env_files/.env.docker.development with your configuration before proceeding!"
        ...
        echo ""
        read -p "Press Enter after updating env_files/.env.docker.development, or Ctrl+C to cancel..."
    else
...
docker-compose --env-file env_files/.env.docker.development build --no-cache

print_header "3. Starting services..."

# Start services with pgAdmin for development
docker-compose --env-file env_files/.env.docker.development --profile tools up -d
...
timeout=60
counter=0
while ! docker-compose --env-file env_files/.env.docker.development exec postgres pg_isready -U healthapp_user -d healthapp_dev > /dev/null 2>&1; do
...
timeout=30
counter=0
while ! docker-compose --env-file env_files/.env.docker.development exec redis redis-cli ping > /dev/null 2>&1; do
    if [ $counter -ge $timeout ]; then
...
docker-compose --env-file env_files/.env.docker.development exec backend npm run migrate || {
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    docker-compose --env-file env_files/.env.docker.development exec backend npm run seed || {
...
for service in "${services[@]}"; do
    if docker-compose --env-file env_files/.env.docker.development ps $service | grep -q "Up"; then
        print_status "$service is running ✓"
    else
        print_error "$service is not running ✗"
        docker-compose --env-file env_files/.env.docker.development logs $service
        exit 1
    fi
done
...
if curl -f http://localhost:3001/health > /dev/null 2>&1; then
    ...
    docker-compose --env-file env_files/.env.docker.development logs backend
fi
...
if curl -f http://localhost:3002 > /dev/null 2>&1; then
    ...
    docker-compose --env-file env_files/.env.docker.development logs frontend
fi

print_header "✅ Development deployment completed!"

echo ""
echo "🌟 HealthApp Development Environment is ready!"
echo ""
echo "📋 Access URLs:"
...
echo "   View logs:        docker-compose --env-file env_files/.env.docker.development logs -f [service]"
echo "   Stop services:    docker-compose --env-file env_files/.env.docker.development down"
echo "   Restart service:  docker-compose --env-file env_files/.env.docker.development restart [service]"
echo "   Shell access:     docker-compose --env-file env_files/.env.docker.development exec [service] sh"
echo ""
...
echo "   Check logs: docker-compose --env-file env_files/.env.docker.development logs"
