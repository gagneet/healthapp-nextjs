    ...
    try {
      ...
      const criticalPatients = await Patient.count({
        where: {
          ...
          [Op.or]: [
            { risk_level: 'high' },
            { overallAdherenceScore: { [Op.lt]: 70 } }
          ]
        }
      });
      ...
      const adherenceResult = await Patient.findOne({
        where: {
          primaryCareDoctorId: doctor.id,
          isActive: true,
          overallAdherenceScore: { [Op.ne]: null }
        },
        attributes: [
          [Patient.sequelize.fn('AVG', Patient.sequelize.col('overallAdherenceScore')), 'avg_adherence']
        ],
        raw: true
      });
  ...
  async getAdherenceAnalytics(req: Request, res: Response, next: NextFunction): Promise<void | Response> {
    try {
      ...
      const medicationAdherence = await Patient.findOne({
        ...
        attributes: [
          [Patient.sequelize.fn('AVG', Patient.sequelize.col('overallAdherenceScore')), 'avg_medication']
        ],
        raw: true
      });
