...
exports.generateHspId = generateHspId;
exports.generatePatientId = generatePatientId;
exports.validateBusinessId = validateBusinessId;
exports.parseBusinessId = parseBusinessId;
exports.generateBulkIds = generateBulkIds;
...
async function generateBusinessId(config) {
    ...
    try {
        ...
        switch (config.tableName) {
            case 'doctors':
                const doctors = await prisma_js_1.prisma.doctors.findMany({
                    where: {
                        doctor_id: {
                ...
                break;
            case 'hsps':
                const hsps = await prisma_js_1.prisma.hsps.findMany({
                    where: {
                        hsp_id: {
                            startsWith: yearPrefix
                        }
                ...
                break;
            case 'patients':
                const patients = await prisma_js_1.prisma.patient.findMany({
                    where: {
                        patient_id: {
                            startsWith: yearPrefix
                        }
 ...
 */
function validateBusinessId(id, expectedPrefix) {
 ...
 */
async function generateBulkIds(count, type) {
    const results = [];
