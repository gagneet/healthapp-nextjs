...
async function createOrganization() {
    // Implementation of organization creation
}

async function createSpecialties() {
    // Implementation of specialties creation
}

async function createDoctorProfiles() {
    // Implementation of doctor profiles creation
}

async function createHSPProfile() {
    // Implementation of HSP profile creation
}

async function createProvider() {
    // Implementation of provider creation
}

async function createPatientProfiles() {
    // Implementation of patient profiles creation
}

async function createMedicines() {
    // Implementation of medicines creation
}

async function createVitalTemplates() {
    // Implementation of vital templates creation
}

async function createSymptomsDatabase() {
    // Implementation of symptoms database creation
}

async function createTreatmentsDatabase() {
    // Implementation of treatments database creation
}
...
export async function seedComprehensiveHealthcareData() {
    ...
    try {
        ...
        await createHSPProfile();
        await createProvider();
        await createPatientProfiles();
        await createMedicines();
        await createVitalTemplates();
        await createSymptomsDatabase();
        await createTreatmentsDatabase();
    ...
    try {
        ...
        await prisma.$transaction(async (prisma) => {
            ...
            await Promise.all([
                ...
                prisma.vital_templates.deleteMany({
                    where: {
                        id: {
...
where: {
        ...
        await prisma.doctors.deleteMany({
            where: {
                user_id: {
                    in: ['99999999-9999-9999-9999-999999999999', '44444444-4444-4444-4444-444444444444']
                }
            }
        });
        await prisma.hsps.deleteMany({
        ...
        await prisma.medicine.deleteMany();
        await prisma.vital_templates.deleteMany();
        await prisma.symptoms_database.deleteMany();
        await prisma.treatment_database.deleteMany();
        console.log('✅ Test data cleared successfully');
    }
    catch (error) {
