        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    // Custom validation: Ensure doctor profile exists for doctor roles
                    if (user.role === 'DOCTOR' && !user.doctorProfile) {
                        console.error(`Login failed: User ${user.email} has DOCTOR role but no doctorProfile linked.`);
                        throw new Error("Doctor account is not configured correctly. Please contact support.");
                    }
