...
async function seedComprehensiveHealthcareData() {
    ...
    try {
        ...
        console.log(`Found ${createdPatients.length} patients for care plans:`, createdPatients.map(p => p.id));
        ...
        // Create care plans with corrected schema (use createMany for simplicity)
        const validCarePlans = carePlanData.filter((planData, i) => 
            i < createdPatients.length && planData.patient_id && createdPatients[i]?.id
        ).map((planData, i) => ({
            patient_id: createdPatients[i].id,
