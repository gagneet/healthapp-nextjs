...
exports.default = {
    up: async (queryInterface, Sequelize) => {
        ...
        const existingUsers = await queryInterface.sequelize.query("SELECT email FROM users WHERE email IN ('admin@healthapp.com', 'doctor@healthapp.com', 'hsp@healthapp.com', 'hospital.admin@healthapp.com', 'patient@healthapp.com')", { type: Sequelize.QueryTypes.SELECT });
        ...
        const existingDoctorUsers = await queryInterface.sequelize.query(`SELECT u.id, u.email FROM users u 
       LEFT JOIN doctors d ON u.id = d.user_id 
       WHERE u.role = 'DOCTOR' AND d.id IS NULL`, { type: Sequelize.QueryTypes.SELECT });
        if (existingDoctorUsers.length > 0) {
            // Ensure General Medicine specialty exists (idempotent)
            await queryInterface.sequelize.query(`INSERT INTO specialities (id, name, description, created_at, updated_at) 
         VALUES (3, 'General Medicine', 'General medical practice', NOW(), NOW())
         ON CONFLICT (id) DO NOTHING`, { type: Sequelize.QueryTypes.INSERT });
        ...
        const existingDoctorsToUpdate = await queryInterface.sequelize.query(`SELECT d.id, d.user_id, u.email FROM doctors d 
       JOIN users u ON d.user_id = u.id 
       WHERE u.role = 'DOCTOR' AND (d.speciality_id IS NULL OR d.specialties = '{}' OR d.is_verified = false)`, { type: Sequelize.QueryTypes.SELECT });
        if (existingDoctorsToUpdate.length > 0) {
         ...
         ON CONFLICT (id) DO NOTHING`, { type: Sequelize.QueryTypes.INSERT });
            for (const doctor of existingDoctorsToUpdate) {
                await queryInterface.sequelize.query(`UPDATE doctors SET 
           speciality_id = 3,
        ...
        const existingPatientUsers = await queryInterface.sequelize.query(`SELECT u.id, u.email FROM users u 
       LEFT JOIN patients p ON u.id = p.user_id 
       WHERE u.role = 'PATIENT' AND p.id IS NULL`, { type: Sequelize.QueryTypes.SELECT });
...
//# sourceMappingURL=000-test-users.js.map