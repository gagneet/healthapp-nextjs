...
exports.default = {
    async up(queryInterface, Sequelize) {
        ...
        try {
            ...
            const existingComprehensiveData = await queryInterface.sequelize.query("SELECT email FROM users WHERE email IN ('patient1@healthapp.com', 'patient2@healthapp.com', 'doctor1@healthapp.com')", { type: Sequelize.QueryTypes.SELECT, transaction });
            ...
            const medicines = await queryInterface.sequelize.query('SELECT id, name FROM medicines LIMIT 10', { type: Sequelize.QueryTypes.SELECT, transaction });
    ...
    async down(queryInterface, Sequelize) {
        ...
        try {
            ...
            await queryInterface.bulkDelete('appointments', {
                patient_id: { [Sequelize.Op.in]: await queryInterface.sequelize.query('SELECT id FROM patients WHERE patient_id IN (?, ?)', {
            ...
            await queryInterface.bulkDelete('medications', {
                participant_id: { [Sequelize.Op.in]: await queryInterface.sequelize.query('SELECT id FROM patients WHERE patient_id IN (?, ?)', {
                        replacements: ['PAT-2024-001', 'PAT-2024-002'],
                        type: Sequelize.QueryTypes.SELECT
                    }).then((results) => results.map((r) => r.id)) }
            }, { transaction, ignoreDuplicates: true });
            await queryInterface.bulkDelete('doctors', {
                user_id: { [Sequelize.Op.in]: await queryInterface.sequelize.query('SELECT id FROM users WHERE email = ?', {
            ...
            await queryInterface.bulkDelete('user_roles', {
                user_id: { [Sequelize.Op.in]: await queryInterface.sequelize.query('SELECT id FROM users WHERE email IN (?, ?, ?)', {
...
//# sourceMappingURL=008-comprehensive-patient-data.js.map