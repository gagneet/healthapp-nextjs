...
exports.default = {
    async up(queryInterface, Sequelize) {
        ...
        const existingChartData = await queryInterface.sequelize.query('SELECT COUNT(*) as count FROM medication_logs', { type: Sequelize.QueryTypes.SELECT });
        ...
        // Get existing data
        const existingPatients = await queryInterface.sequelize.query('SELECT p.id, p.user_id FROM patients p JOIN users u ON p.user_id = u.id WHERE u.deleted_at IS NULL LIMIT 20', { type: Sequelize.QueryTypes.SELECT });
        const existingDoctors = await queryInterface.sequelize.query('SELECT d.id, d.user_id FROM doctors d JOIN users u ON d.user_id = u.id WHERE u.deleted_at IS NULL LIMIT 10', { type: Sequelize.QueryTypes.SELECT });
        const existingMedicines = await queryInterface.sequelize.query('SELECT id, name FROM medicines LIMIT 50', { type: Sequelize.QueryTypes.SELECT });
        ...
        for (let i = 0; i < existingPatients.length && i < 15; i++) {
            ...
            for (const vitalType of vitalTypes) {
                ...
                for (let day = 0; day < 30; day += Math.floor(Math.random() * 2) + 2) {
                    ...
                    const vitalTypeId = await queryInterface.sequelize.query(`SELECT id FROM vital_types WHERE name ILIKE '%${vitalType.type}%' LIMIT 1`, { type: Sequelize.QueryTypes.SELECT });
...
//# sourceMappingURL=20250807041830-comprehensive-chart-test-data.js.map