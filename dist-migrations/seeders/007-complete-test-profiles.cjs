...
exports.default = {
    up: async (queryInterface, Sequelize) => {
        ...
        let users = await queryInterface.sequelize.query("SELECT id, email, role FROM users WHERE email IN ('admin@healthapp.com', 'doctor@healthapp.com', 'patient@healthapp.com')", { type: Sequelize.QueryTypes.SELECT });
        ...
        users = await queryInterface.sequelize.query("SELECT id, email, role FROM users WHERE email IN ('admin@healthapp.com', 'doctor@healthapp.com', 'patient@healthapp.com')", { type: Sequelize.QueryTypes.SELECT });
        ...
        if (doctorUser) {
            const existingDoctor = await queryInterface.sequelize.query("SELECT id FROM doctors WHERE user_id = :userId", {
        ...
        if (patientUser && doctorUser) {
            const existingPatient = await queryInterface.sequelize.query("SELECT id FROM patients WHERE user_id = :userId", {
                replacements: { userId: patientUser.id },
                type: Sequelize.QueryTypes.SELECT
            });
            if (existingPatient.length === 0) {
                // Get the doctor ID first
                const doctorRecord = await queryInterface.sequelize.query("SELECT id FROM doctors WHERE user_id = :userId", {
                    replacements: { userId: doctorUser.id },
                    type: Sequelize.QueryTypes.SELECT
                });
    ...
    down: async (queryInterface, Sequelize) => {
        ...
        await queryInterface.bulkDelete('clinics', {
            doctor_id: {
                [Sequelize.Op.in]: queryInterface.sequelize.literal("(SELECT d.id FROM doctors d JOIN users u ON d.user_id = u.id WHERE u.email = 'doctor@healthapp.com')")
            }
        });
        await queryInterface.bulkDelete('patients', {
            user_id: {
                [Sequelize.Op.in]: queryInterface.sequelize.literal("(SELECT id FROM users WHERE email = 'patient@healthapp.com')")
            }
        });
        await queryInterface.bulkDelete('doctors', {
            user_id: {
                [Sequelize.Op.in]: queryInterface.sequelize.literal("(SELECT id FROM users WHERE email = 'doctor@healthapp.com')")
            }
        });
...
//# sourceMappingURL=007-complete-test-profiles.js.map